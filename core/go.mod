module github.com/nexus/nexus-core

go 1.26.0

// PINNED. Core version bumps are a reviewed change — see Nexus/CLAUDE.md.
// Upstream perf regressions are the main threat to the product thesis (ADR-0001 §5.6).
//
// libbox lives in-tree in this module (experimental/libbox), which is why we get the
// binding and the core from one pin. That was a selection criterion (ADR-0001 §3.4).
require github.com/sagernet/sing-box v1.14.0

require github.com/sagernet/sing v0.9.0-beta.4 // indirect; indirect — version must match the sing-box pin
